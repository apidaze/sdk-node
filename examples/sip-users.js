...
(async () => {
  ...
  // update a user
  const { body: updatedUser } = await ApidazeClient.sipUsers.update(
    createdUser.id,
    {
  ...
  const {
    body: userStatus,
    statusCode,
  } = await ApidazeClient.sipUsers.getStatus(user.id);
  ...
  const {
    body: password,
    statusCode: ss,
  } = await ApidazeClient.sipUsers.resetPassword(user.id);
  console.log('the new password', password, ss);

  // delete a user
  const deletedUserResponse = await ApidazeClient.sipUsers.delete(
    createdUser.id
  );
